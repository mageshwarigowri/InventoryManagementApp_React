...
const Dashboard = ({ inventory, categories, theme, isDarkMode }) => {
  const metrics = {
    ...
    expectedProfit: inventory.reduce((sum, item) => sum + (Number(item.quantity) * (Number(item.price) - Number(item.costPrice || 0))), 0) // NEW PROFIT CALCULATION
  };
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in duration-500 no-print">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className={`${theme.bgCard} p-6 rounded-xl border ${theme.border} shadow-sm border-l-4 border-l-green-500`}>
          <span className={`${theme.textMuted} text-sm font-medium mb-1 block`}>Expected Profit</span>
          <span className={`text-3xl font-bold text-green-600 dark:text-green-400`}>₹{metrics.expectedProfit.toLocaleString()}</span>
        </div>
        <div className={`${theme.bgCard} p-6 rounded-xl border ${theme.border} shadow-sm`}>

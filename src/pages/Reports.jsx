...
const Reports = ({ inventory, theme }) => {
  const exportToCSV = () => {
    ...
    const rows = inventory.map(i => `${i.productId},${i.name},${i.category},${i.quantity},${i.unit},${i.price},${i.gst},${i.barcode},${i.quantity*i.price},${i.mfgDate},${i.expDate}`);
    const link = document.createElement("a");
    link.href = encodeURI("data:text/csv;charset=utf-8," + headers.concat(rows).join("\n"));
    link.download = "inventory_report.csv"; link.click();
  };
